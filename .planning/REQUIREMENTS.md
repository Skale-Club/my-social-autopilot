...
### Image Provider Abstraction (PROV)

- [ ] **PROV-01**: A `server/services/image-provider.ts` module exports an `ImageProvider` TypeScript interface with `generate()` and `edit()` methods, plus a `getActiveImageProvider()` factory function that returns the concrete provider implementation based on `platform_settings.image_provider`
- [ ] **PROV-02**: An `OpenAIImageProvider` implementation uses OpenAI's Responses API (`tools: [{type: "image_generation"}]`, model `gpt-image-2`) — not the legacy Image API — to support multi-image reference inputs and edit flows
- [ ] **PROV-03**: Reference images passed to providers are correctly format-converted: Gemini `{mimeType, data}` ↔ OpenAI `{type: "input_image", image_url: "data:{mime};base64,{data}"}`; the conversion is unit-tested
- [ ] **PROV-04**: `platform_settings` has an `image_provider` row (values `'gemini'` or `'openai'`, default `'gemini'`); reading/writing uses the existing `getPlatformSetting` / `setPlatformSetting` pattern from `app-settings.service.ts`
- [ ] **PROV-05**: Admin settings page has an "AI Image Provider" section with a radio toggle between Gemini and OpenAI; selecting and saving writes to `platform_settings.image_provider` and takes effect on the next generation request without server restart
- [ ] **PROV-06**: API key resolution adds support for `OPENAI_API_KEY` (server env, for regular users) and `profiles.openai_api_key` (per-admin/affiliate key); resolution centralized in `auth.middleware.ts` alongside Gemini key resolution
- [ ] **PROV-07**: All four generation flows (single image generate, edit-post, carousel slides, enhancement) route through the provider factory; switching providers in admin UI produces working output from BOTH providers across ALL four flows without code changes
...
| GLRY-04 | Phase 10 | Complete |
| GLRY-05 | Phase 10 | Complete |
